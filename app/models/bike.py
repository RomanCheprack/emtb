...
class Comparison(Base):
    ...
    def get_bike_ids(self):
        ...
        import json
        try:
            return json.loads(self.bike_ids) if self.bike_ids else []
        except (json.JSONDecodeError, TypeError):
            return []
    
    def set_bike_ids(self, bike_ids_list):
        """Set bike IDs as JSON string"""
        self.bike_ids = json.dumps(bike_ids_list)
    ...
    def set_comparison_data(self, data_dict):
        ...
        self.comparison_data = json.dumps(data_dict, ensure_ascii=False)
    
    def generate_slug(self, bike_ids_list, session):
        """Generate SEO-friendly slug from bike IDs"""
        bike_slugs = []
        
        if session and bike_ids_list:
